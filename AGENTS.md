...
XSD validation searches `documentation/eCH-0196-2-2.xsd` beside the script or
frozen executable first, then in the working directory. Download the imported
schemas too, with local `schemaLocation` references. `documentation/` is
git-ignored local scratch space. Missing `lxml` or XSD causes explicit skip.
